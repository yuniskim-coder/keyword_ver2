...
def show_rank_checker():
    ...
    if search_button and keywords_text and seller_name:
        ...
        # 사용법 안내
        with st.expander("💡 사용법 및 팁", expanded=False):
            st.markdown("""
            ### 📋 순위 확인기 사용 방법
            ...
            **1️⃣ 검색어 입력**
            - 찾고자 하는 상품의 키워드를 입력
            - 여러 키워드는 쉼표(,)로 구분
            - 예: `무선키보드, 게이밍키보드, 블루투스키보드`
            
            **2️⃣ 판매처명 입력**
            - 네이버 쇼핑에 표시되는 정확한 판매처명 입력
            - 대소문자, 띄어쓰기 정확히 일치해야 함
            - 예: `쿠팡`, `11번가`, `옥션`
            
            **3️⃣ 검색 옵션 설정**
            - **최대 검색 순위**: 검색할 최대 순위 (100~1000위)
            - **정렬 방식**: 정확도순, 날짜순, 가격순 선택
            
            ### 💡 검색 팁
            - 🎯 **구체적인 키워드**: 너무 광범위하지 않게
            - 📝 **정확한 판매처명**: 네이버 쇼핑 화면의 정확한 이름
            - 🔄 **다양한 키워드**: 동일 상품의 여러 키워드 테스트
            - ⏱️ **검색 시간**: 순위가 높을수록 검색 시간 단축
            """)
